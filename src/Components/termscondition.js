...
componentDidMount() {
this.getcoartcondition()
this.getcoartname()

}
async getcoartcondition() {
    await axios.post(`${config.apiUrl}getcoartcondition`,{'coartid':this.coartid},)
        .then(result => {
            ...
            if (result.data.success === true) {
                 this.setState({
                    termscondition:result.data.response
                })
            }
            else{
              this.setState({
...
let  coartid= this.coartid;
  ...
  axios.post(`${config.apiUrl}insertcoartcondition`, obj)
    .then(response => {
      ...
      buttons: [
          {
              ...
              axios({
                  method: 'post',
                  url: `${config.apiUrl}deletecoartcondition`,
                      ...
                      <div className="dual-list-box-inner">
                        <div className="row">
                           ...
                           <h3>{this.state.coart_name}</h3>
                                  <input type="text" class="form-control" name="terms"  value={this.state.terms} onChange={this.handleChange} id="recipient-name"/> <br/>
                                ...
                                <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true" data-cookie-id-table="saveId" data-show-export="true" data-click-to-select="true" data-toolbar="#toolbar" className="table table-bordered table-striped table-hover">
                                  <thead>
                                    <tr>
                                      ...
                                      <th data-field="name" data-editable="true">Terms & Condition</th>    
                                      <th data-field="action">Action</th>
                                      
                                    </tr>
                                  </thead>
                                  <tbody>
                                      {this.state.termscondition.map((item,i) => (
                                      <tr>
                                        <td>{i+1}</td>
                                        <td>{item.terms} </td>
                                        <button classsName=" btn-danger"  data-toggle="tooltip" onClick={this.deletecoartcondition.bind(this,item.id)} data-original-title="Close"> <i class="fa fa-close m-r-10"></i> 
                                        </button>
